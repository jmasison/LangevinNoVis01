...
import g.counter.GStateCounter;
...
import g.reaction.GAllostericReaction;
...
public class Global {
    ...
    // Pass in 3rd variable to designate a file for the System.out outputstream
    // I use this to keep track of my simulation progress!  We pass one in if
    // we're launching from LangevinSetup (that's the gui).  If we're on the 
    // command line we'll make our own.
    public static void main(String [] args) {
        // System.out.println("Main directory: " + System.getProperty("user.dir"));
        Global g;
        MySystem sys;
        if(args.length == 2){
            File file = new File(args[0]);
            g = new Global(file);
            sys = new MySystem(g, args[1], false);
        } else {
            File file = new File(args[0]);
            File outFile = new File(args[2]);
            g = new Global(file, outFile);
            sys = new MySystem(g, args[1], true);
        }
